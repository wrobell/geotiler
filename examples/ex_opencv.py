...
class MapObject:
    def __init__(self,center:tuple=(0.0,51.47879),zoom:int=15,size:tuple=(1900,1000)):
        """
            MapObject constructor
            Creates a open cv compatible map image of requested size, zoom level
            and coordinates. After creating the object the map can be displayed
            using cv2.imShow('window name',MapObject.img) and cv2.waitKey(10)

            Keyword arguments:
                center (Optional[tuple]) -- center coordinates of the map tuple(lon,lat) (default 0.0,51.47879) (Greenwich)
                zoom (Optional[int]) -- zoom level of the map, int between 3 and 19 (default 15)
                size (Optional[int]) -- resolution of the resulting image (default (1900,1000)) (good for full HD monitor)
    ...
    def drawMap(self):
        ...
        """
        self.img=cv2.cvtColor(np.array(self.pilImage)[:,:,:3], cv2.COLOR_RGB2BGR)
    ...
    def plotPoint(self,markers):
        ...
        for lon,lat in markers:
            ...
            cv2.circle(self.img,center=(int(x),int(y)), radius=5,color=RED, thickness=-1)
    ...
    def mouse_callback(self,event,x,y,flag=0, param=None):
        """
            ...
            Scrolling will zoom in or out at the location the mouse is pointing at
        """
...
kaart=MapObject()
# Create a window called window and have it adjust in size depending the image size
...
while 1:
    ...
    if (key == 27): # when Esc is pressed: close all opencv windows and break from the while loop
